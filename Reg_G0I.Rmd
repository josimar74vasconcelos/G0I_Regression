  ...
  ll <- function(par){
    ...
    Eta   <- crossprod(t(X), beta)
     ...
     dG0I <- lgamma(L-alpha) - alpha * log(-alpha-1) - 
             alpha * mean(log(ginv(Eta))) - 
             lgamma(-alpha) + (alpha-L) * mean(log((-alpha-1)*ginv(Eta) + L*Y))
      return(dG0I)
  } 
  return(ll)
}
...
```{r, warning = FALSE, message=TRUE, include=TRUE}
set.seed(241011)
RES <- MC.SIM(NREP=1000, theta=c(-3.5, 1.7, 1.5, 8), Dist = "Unif")      
